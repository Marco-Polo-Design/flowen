...
function headerBlack() {
    $(document).on('scroll', () => {
        ...
        const backgroundTrigger = document.querySelector('.animatable-text').offsetTop;


        console.log(pixelsForHeader);
        console.log(backgroundTrigger);


        if ($('header').hasClass('no-background-really') && pixelsForHeader < backgroundTrigger) {
            $('header').css('background-color', '');
        } else if (pixelsForHeader > backgroundTrigger) {
...
function openProductChosen() {
    ...
    if ($(window).width() > 768) {
        ...
        $('.product-variants a').on('click', event => {
            ...
            $(productChosenLines).addClass('clicked');
    ...
    if ($(window).width() > 768) {
        $('.product-chosen-options a').on('click', () => {
            ...
            $('.add-to-cart-button').css('width', '');
            $('.add-to-cart-bcgd').removeClass('animate-up');
            $('.add-to-cart-button-inner p').css('color', '');

            $('.add-to-cart-bcgd-gold').css('opacity', '0');
            $('.add-to-cart-button').css('cursor', '');

            $('div.sizes a').removeClass('size-is-active')
...
function openCart() {
    ...
    const bagCTA = document.querySelectorAll('.bag-cta-container');
...
function addToCartAnimations() {
    $('div.sizes a').on('click', function () {
        if ($('div.sizes a').hasClass('size-is-active') && $('a.product-variant').hasClass('selected')) {
            $('.add-to-cart-bcgd').addClass('animate-up');
            $('.add-to-cart-bcgd-gold').css('opacity', '1');
            $('.add-to-cart-button-inner p').css('color', '');
            $('.add-to-cart-button').css('cursor', 'pointer');
        }
    })
